         ...
         <div class="form-group">
             ...
             <div class="col-sm-9">
                 <div class="form-control-box">
                     ...
                     <input type="text" class="form-control" name="{{$row->productEavToEav->id}}" maxlength="{{$row->productEavToEav->length}}" @if( $row->productEavToEav->is_required == 0 ) ignore="ignore" @endif  datatype="{{$row->productEavToEav->valid_rule}}"  errormsg="{{$row->productEavToEav->errormsg}}" tipsrmsg="{{$row->productEavToEav->tipgs}}" >
                     <span class="Validform_checktip"></span>
                     @endif

                     <!-- time -->
                     @if( $row->productEavToEav->front_input == 'time' )
                     <input type="text" class="form-control" onfocus="WdatePicker({lang:'zh-cn',minDate:new Date(new Date().valueOf())})"  name="{{$row->productEavToEav->id}}"  @if( $row->productEavToEav->is_required == 0 ) ignore="ignore" @endif  datatype="{{$row->productEavToEav->valid_rule}}"  errormsg="{{$row->productEavToEav->errormsg}}" tipsrmsg="{{$row->productEavToEav->tipgs}}" >
                     <span class="Validform_checktip"></span>
                     @endif
                     ...
                     <select class="form-control valid w200" name="{{$row->productEavToEav->id}}" >
                         @foreach(Source_Eav_AttrbuteValue::where('attributeid',$row->productEavToEav->id)->get()  as $rows )
                             <option value="{{$rows->value}}">{{$rows->text}}</option>
                         @endforeach
                     </select>
                     <span class="Validform_checktip"></span>
                     @endif
                     ...
                     @if( $row->productEavToEav->front_input == 'textarea' )
                         <textarea class="form-control valid"  maxlength="{{$row->productEavToEav->length}}" @if( $row->productEavToEav->name == 'desc') id="desc" @endif rows="5" name="{{$row->productEavToEav->id}}"  @if( $row->productEavToEav->is_required == 0 ) ignore="ignore" @endif  datatype="{{$row->productEavToEav->valid_rule}}"  errormsg="{{$row->productEavToEav->errormsg}}" tipsrmsg="{{$row->productEavToEav->tipgs}}"></textarea>
                         <span class="Validform_checktip"></span>
                     @endif
                 </div>
             </div>
                 ...
                 <div class="form-control-box Father_Item{{$Keys}}">
                     ...
                     <label class="control-label cur-p m-r-10">
                         ...
                         <span title="{{$value->text}}">
                             @if( $value->images )
                             <img src="{{getImagesUrl('attribute',$value->attributeid,$value->images)}}" data-value="[img]attribute/{{$value->attributeid.'/'.$value->images}}"  data-v="{{$value->text}}"  height="32" width="32">
                             @else
                             {{$value->value}}
                             @endif
                         </span>
