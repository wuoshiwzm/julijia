                ...
                <div class="simple-form-field">
                    <div class="form-group">
                        ...
                        <div class="form-control-wrap">
                            <input id="searchmodel-keyword" class="form-control" name="keyword" placeholder="请输入角色名称" type="text" value="{{$set['keyword']}}">
                        </div>
                    </div>
                </div>
                ...
                @foreach($groups as $row)
                    <tr>
                        ...
                        <td class="handle">
                            ...
                            <span>|</span>
                            <a href="javascript:void(0);" class="offsale-goods dele_user" onclick="deleteSuite('{{encode($row->group_id)}}')">删除</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
    ...
    <script>
        function  deleteSuite(id) {
            layer.open({
                ...
                shadeClose: false,
                yes: function(){
                    $.post('/admin/suite/delete',{group_id:id},function (msg) {
                        msg=eval("("+msg+")") ;
                        if(msg['status']){
                            location.reload();
                        }else{
                            layer.alert('此角色下存在用户，请删除用户后再次操作！');
                        }
                    })
                }, no: function(){
                    layer.close();
