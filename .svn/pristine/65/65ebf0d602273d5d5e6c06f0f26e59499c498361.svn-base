    ...
    <div class="search-term m-b-10">
        <form action="{{Request::url()}}" method="get">
            ...
            <span class="action-span">
				<a href="{{url('admin/product/goods/create')}}" class="btn btn-warning click-loading"> <i class="fa fa-plus"></i>发布商品</a>
			</span>
        </form>
    </div>
            ...
            <tr>
                ...
                <td>
                    <div class="goodsPicBox pull-left m-r-10">
                        <a  @if( $row->status == 1) href="{{url($row->entity_id.'.html')}}"  target="_blank" @else href="javascript:;" @endif>
                    ...
                    <div class="ng-binding goods-message w200">
                        <div class="name">
                            <a @if( $row->status == 1) href="{{url($row->entity_id.'.html')}}" target="_blank"   @else href="javascript:;" @endif data-toggle="tooltip" data-placement="auto bottom">{{$row->name}}</a>
                        </div>
                    </div>
                </td>
                ...
                <td class="handle">
                    @if( $row->status == 1)
                    <a href="{{url($row->entity_id.'.html')}}" target="_blank">查看</a>
                    <span>|</span>
