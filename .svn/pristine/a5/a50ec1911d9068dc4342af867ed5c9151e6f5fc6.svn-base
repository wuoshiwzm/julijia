...
class Upload {
       ...
       try{
           switch ( $uploadtype )
           {
               ...
               case "config":
                   ...
                   break;
               case  "refund":
                   $filepath  = $this->filepath.'/refund/';
                   break;
               case "goods/cache":
                   $filepath  = $this->filepath.'/goods/cache/';
                   break;
           }
     ...
     * 下载文件
     */
    public function download( $dir, $id, $name, $newName )
    {
        $ftpPath = $this->filepath.'/'.$dir.'/'.$id.'/'.$name;
        $temPath = public_path().$this->fileFrom.$newName;
        return $this->connect->downloadFile( $ftpPath, $temPath);
    }
}