		...
		if( $methodType == 'POST' )
		{
			...
			if ( $res )
			{
				...
				if( $data['type'] == 'varchar' )
				{
					if( $data['is_system'] == 1 )
					{
						DB::select('ALTER TABLE shop_product_flat MODIFY COLUMN '.$data['name'].' VARCHAR('.$data['length'].')');
					}else
					{
						DB::select('ALTER TABLE shop_product_flat_detail MODIFY COLUMN '.$data['name'].' VARCHAR('.$data['length'].')');
					}
				}
				//修改成功
