		...
		<form  action="{{Request::url()}}" method="get">
			...
			<div class="simple-form-field">
				<div class="form-group">
					...
					<div class="form-control-wrap">
						<select id="##" class="form-control" name="news_cate_id">
							<option value="">不限制</option>
							@foreach($cate as $a=>$b)
							<option value="{{$b->news_cate_id}}">{{$b->news_cate_name}}</option>
							@endforeach
						</select>
					</div>
				</div>
			...
			<div class="simple-form-field">
				<div class="form-group">
					...
					<div class="form-control-wrap">
					<input type="text" id="searchmodel-keyword" class="form-control" name="news_title">
					</div>
			...
			<div class="simple-form-field">
				<div class="form-group">
					...
					<div class="form-control-wrap">
						<select id="##" class="form-control" name="news_available">
							<option value="">不限制</option>
			...
			<div class="simple-form-field">
				...
				<input type="button" id="btn_search" value="新增新闻" class="btn btn-primary m-r-5" onclick="location='{{url('admin/newsart/create')}}'">
			</div>
		...
		<table id="table_list" class="table table-hover">
			<thead>
				<tr>
                    ...
                    <th class="w40 tcheck" style="cursor: pointer;"> <input type="checkbox" class="checkbox table-list-checkbox-all" title="全选/全不选"></th>
			...
			<tbody>
				...
				<tr>
                    <td class="tcheck">
						<input type="checkbox" class="checkbox table-list-checkbox" value="349">
					</td>
					<td>
						{{$v->news_id}}
					</td>
					...
					<td>
                @foreach($cate as $a=>$b)
									@if($b->news_cate_id == $v->news_cate_id)
										{{$b->news_cate_name}}
									@endif
								@endforeach
          </td>
          ...
          {{$v->updated_at}}
					...
					<td class="handle">
							...
							<a href="{{url('admin/newsart/'.encode($v->news_id).'/edit')}}">编辑</a>&nbsp;&nbsp;
							<a href="javascript:;" class="dele_d" onclick="delNews({{"'".encode($v->news_id)."'"}});">删除</a>
            ...
            <tfoot>
				<tr>
					...
					<td colspan="4">
						<div class="pull-left">
                        	<input type="button" class="btn btn-danger m-r-2 batch-offsale-goods" value="批量删除">
