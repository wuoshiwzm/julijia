...
class ProductEav
{
    ...
    static function editAb( $data, $id )
    {
        $result = DB::transaction(function() use ( $data, $id )
        {
            ...
            //修改检索表
            if( $data['is_layer_search'] == '0')
            {
                $OptionID = Source_Screen_CategoryOption::where('attbute_id',$id)->lists('id');
                Source_Screen_CategoryOptionDetail::whereIn('option_id',$OptionID)->delete();
                Source_Screen_CategoryOption::where('attbute_id',$id)->delete();
            }
            //更新
            if ( $editId )
            {
    ...
    {
        $baseID = Source_Eav_Attrbute::where('is_system',1)->select('id')->lists('id');
        //排除系统属性
        $fEav = Source_Product_ProductEav::where( 'attbute_set_id', $id )->whereNotIn('attbute_id',$baseID)->with('productEavToEav')->get();
        $res = array(
        ...
        if( count($fEav) )
        {
            $res = ProductEav::getBaseAttrbute( $res );
            $aId = array();
     ...
     * 系统属性 属性集
     */
    static  function getBaseAttrbute( $res )
    {
