...
class Complain
{
    ...
    {
        $model = Source_Feedback_FeedbackInfo::join("userinfo","feedback_info.user_id","=","userinfo.id")
            ->join("userinfo_group","userinfo.group_id","=","userinfo_group.id");
        if ($status != 0 && is_int($status)) {
            $model->where("feedback_info.status","=",$status);
        }
    ...
    {
        $id = (int) $id;
        return  Source_Feedback_FeedbackInfo::find($id);
    }
